...
zone=$(basename $temp)
FW_IP=$(gcloud compute instances describe firewall --zone=$zone --format="value(networkInterfaces[2].networkIP)")
FW_PublicIP=$(gcloud compute instances describe firewall --zone=$zone --format="value(networkInterfaces[1].accessConfigs[0].natIP)")
DB_IP=$(gcloud compute instances describe dbserver --zone=$zone --format="value(networkInterfaces[0].networkIP)")
while true
  do
...
wget -O /usr/lib/cgi-bin/guess-sql-root-password.cgi https://raw.githubusercontent.com/PaloAltoNetworks/azure/master/two-tier-sample/guess-sql-root-password.cgi
...
wget -O /usr/lib/cgi-bin/ssh-to-db.cgi https://raw.githubusercontent.com/PaloAltoNetworks/azure/master/two-tier-sample/ssh-to-db.cgi
chmod +x /usr/lib/cgi-bin/ssh-to-db.cgi
sed -i "s/DB-IP-ADDRESS/$DB_IP/g" /usr/lib/cgi-bin/ssh-to-db.cgi
wget -O /var/www/html/sql-attack.html https://raw.githubusercontent.com/PaloAltoNetworks/azure/master/two-tier-sample/sql-attack.html
